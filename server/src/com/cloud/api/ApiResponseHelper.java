...
public class ApiResponseHelper implements ResponseGenerator {
    ...
    public SnapshotResponse createSnapshotResponse(Snapshot snapshot) {
        ...
        if (volume != null) {
            ...
            snapshotResponse.setVolumeSize(volume.getSize());
    ...
    public LoadBalancerResponse createLoadBalancerResponse(LoadBalancer loadBalancer) {
        ...
        List<LoadBalancerOptionsVO> lbOptions = ApiDBUtils.listLoadBalancerOptions(loadBalancer.getId());
        if (lbOptions != null) {
            for (LoadBalancerOptionsVO lbOption : lbOptions) {
                if (lbOption.getLoadBalancerId() == loadBalancer.getId()) {
                    ...
                    lbResponse.setHealthCheckDestination(lbOption.getHealthCheckDestination());
        ...
        if (!configs.isEmpty()){
            ...
            if (target != null) {
                ...
                s_logger.warn("Load balancer target not found, please remove config " + linkedConfig.getId());
            }
        }
    ...
    */
    ...
    public ServiceResponse createNetworkServiceResponse(Service service) {
        ...
        for (Network.Provider serviceProvider : serviceProviders) {
            ...
            if (service == Service.Firewall
                    && !(serviceProvider == Provider.VirtualRouter || serviceProvider == Provider.JuniperSRX || serviceProvider == Provider.CiscoVnmc || serviceProvider == Provider.PaloAlto || serviceProvider == Provider.NuageVsp || serviceProvider == Provider.GloboAclApi)) {
                continue;
            }
    ...
    public AutoScalePolicyResponse createAutoScalePolicyResponse(AutoScalePolicy policy) {
        ...
        response.setLogicalOperator(policy.getLogicalOperator().name());
